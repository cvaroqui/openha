...
check_node(gchar * name)
{
	...
	if (!found) {
		fprintf(stderr,
			"Node %s not found in EZ_NODES configuration file.\n",
			name);
		return FALSE;
	} else {
#ifdef DEBUG
...
gboolean
check_interface(gchar * type, gchar * interface, gchar * address, gchar * port)
{
	gint rc;
	struct in_addr mreq;
	FILE *File;
...
gboolean
check_disk_addr(gint value)
{
	if ((value < 0) || (value > 1024)) {
		fprintf(stderr,
			"BAD block offset. Must be between 0 and 1023.\n");
		return FALSE;
	} else {
#ifdef DEBUG
...
gboolean
check_port(gint value)
{
	if ((value <= 1025) || (value >= 65535)) {
		fprintf(stderr,
			"BAD port number: %d. Must be between 1025 and 65535.\n",
			value);
		return FALSE;
	} else {
#ifdef DEBUG
...
gboolean
check_timeout(gchar * timeout)
{
	guint value;
	sscanf((const char *) timeout, "%d", &value);
	if ((value < 2) || (value > 120)) {
		fprintf(stderr,
			"BAD timeout value. Must be between 2 and 120 seconds.\n");
		return FALSE;
	} else {
#ifdef DEBUG
